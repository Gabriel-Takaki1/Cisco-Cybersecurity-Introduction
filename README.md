# 🛡️ Cisco Cybersecurity Introduction
![Status](https://img.shields.io/badge/Status-Conclu%C3%ADdo-brightgreen) ![Plataforma](https://img.shields.io/badge/Plataforma-Cisco%20Networking%20Academy-blue)

Este repositório foi criado para registrar a conclusão do curso de **Introdução à Cibersegurança** da Cisco Networking Academy. O objetivo é documentar o início da minha trajetória e os fundamentos adquiridos na área de segurança da informação. 🚀

---

### 📚 Conhecimentos Desenvolvidos:

* 🔎 **Cenário de Ameaças:** Identificação de malwares, ataques de phishing e táticas de engenharia social.
* 🔐 **Proteção de Dados:** Conceitos fundamentais de criptografia, privacidade e integridade da informação.
* 🌐 **Segurança em Rede:** Princípios básicos de defesa cibernética e o funcionamento de firewalls.

---

### 🏅 Certificação

A credencial oficial foi emitida através da plataforma Credly e pode ser validada no link abaixo:

🔗 [Verificar credencial na Credly](https://www.credly.com/badges/a2476185-793f-4bb0-bba1-55d2fea12e6c)
<img width="339" height="338" alt="image" src="https://github.com/user-attachments/assets/76f34985-eba2-4800-9aaa-2cfe934724f4" />
<p align="center">
  <img src="certificado.png" alt="Cisco Cybersecurity Badge" width="250">
</p>
